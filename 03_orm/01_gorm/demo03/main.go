...
import (
	"database/sql"
	"fmt"
	"github.com/magicgopher/gopher/03_orm/01_gorm/demo03/models"
...
func main() {
	...
	//使用AutoMigrate根据结构体来创建新表
	err = setAutoMigrate(models.User{})
	if err != nil {
		log.Fatal("自动迁移schema失败:", err)
	}
	fmt.Println("自动迁移schema成功!")
}

// setAutoMigrate 自动迁移 schema
func setAutoMigrate(dst any) error {
